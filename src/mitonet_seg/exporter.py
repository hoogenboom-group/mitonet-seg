...
def remote_export(url, token, new_dataset_name, layer_name, voxel_size, mag, bbox, 
                  seg, layers_2_link, dtype_per_layer="uint16", downsample=False):
    ...
    with wk.webknossos_context(
        ...
        with TemporaryDirectory() as tempdir:
            ...
            mito_layer = new_dataset.add_layer(
                layer_name,
                wk.SEGMENTATION_CATEGORY,
                dtype_per_layer=dtype_per_layer,
                compressed=True,
            ...
            mito_layer.bounding_box = bbox.align_with_mag(mag) # Highest MAG
            mito_layer.add_mag(mag, compress=True).write(seg)
            
            # Downsample segmentation
            if downsample:  
                mito_layer.downsample(
