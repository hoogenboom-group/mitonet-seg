...
USE_CPU = False # Use GPU, seting to "True" falls back to CPU (computations are much slower)
...
def _main():
    ...
    if REMOTE:
        ...
        # Import full data set at required zoom level
    ...
    # Define chunks if array size exceeds vram
    ...
    if REMOTE:
        ...
        exporter.remote_export(url=URL, token=TOKEN, new_dataset_name=new_dataset_name, layer_name=new_layer_name, voxel_size=voxelsize, mag=MAG, 
                               bbox=layer_bbox, seg=mito_labels, layers_2_link=layers_2_link, dtype_per_layer="uint16", downsample=DOWNSAMPLE)
    else:
        exporter.local_export(dataset=dataset, layer_name=new_layer_name, mag=MAG, dtype_per_layer="uint16")
